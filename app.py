...
from segmentation import video_segmentation
import segmentation
import cv2
...
def generate_processed_frames(conf_=0.25):
    ...
    yolo_output = video_segmentation(conf_, frames_buffer)
    # iterate through the list of processed frames
    for segment in yolo_output:
    ...
    if request.method == "POST":
            ...
            # check if the uploaded file is a video
                ...
                if video.content_length > 200 * 1024 * 1024:
                    ...
                    try:
                        ...
                        # print(e)
                        return f"Error! The video could not be saved due to following error: \n {e}"
        ...
        if 'download_button' in request.form:
            ...
            with open('static/reports/detections_summary.txt', 'w') as f:
                f.write(segmentation.detections_summary)
        ...
        # handle alert email request
            ...
            if email_checkbox_value == 'false':
                segmentation.is_email_allowed = False
                return "Alert email is disabled"
            else:
                # set flag that sending email alert is allowed
                segmentation.is_email_allowed = True
                # set flag that next email can be sent when a violation is detected
                segmentation.send_next_email = True
                segmentation.email_recipient = request.form['alert_email_textbox']
                return f"Alert email is enabled at {segmentation.email_recipient}. Violation alert(s) with a gap of " \
                       f"10 minutes will be sent"
